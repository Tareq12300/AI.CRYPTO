...
async def analyze_batch(batch: list[str]) -> list[dict]:
    prompt = (
        ...
        "لكل عملة أعطني سطراً واحداً بهذا التنسيق الحرفي:\n"
        "SYMBOL|شراء|سبب مفصّل للشراء في 15 كلمة عربية|عالي أو متوسط أو منخفض\n\n"
        "قواعد صارمة:\n"
        "- فقط توصيات الشراء — إذا لم تكن العملة مناسبة للشراء اكتب: SYMBOL|تخطي||\n"
        "- اذكر السبب بشكل مفصّل: المؤشرات الفنية، الزخم، مستوى الدعم، حجم التداول\n"
        "- لا تكتب أي نص إضافي خارج الأسطر\n"
        "- مثال: BTC|شراء|اختراق مقاومة 70k مع RSI صاعد وحجم تداول مرتفع وزخم قوي|عالي"
    )
